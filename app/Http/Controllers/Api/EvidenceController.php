...
{
    public function __construct(private readonly EvidenceRepository $repository)
    {
    }
    ...
    public function forSignal(\Illuminate\Http\Request $request, string $tenantId, string $signalId): \Illuminate\Http\JsonResponse
    {
        $all = $this->repository->list($this->tenantId($request));

        return response()->json(array_values(array_filter(
            $all, fn ($e) => ($e['signal_id'] ?? null) === $signalId
        )));
    }

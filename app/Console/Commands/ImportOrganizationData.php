...
final class ImportOrganizationData extends Command
{
    ...
    public function handle(
        ...
        if ($this->option('generate-signals') && ! $dryRun) {
            ...
            // Two families, one loop. The rules held as rows in
            // hpbrain_signal_rules are evaluated by RuleEvaluator exactly as a
            // scheduled detection run would; the operational rules, which are
            // aggregates a row cannot express, are attached by the datasets
            // this tenant actually holds. Both write through the same signal
            // path, so nothing downstream can tell them apart.
            $result = app(RuleEvaluator::class)->evaluate($tenant);
            ...
            foreach (app(SignalRuleRegistry::class)->extraRulesFor($writer, $tenant) as $rule) {
                try {
                    $outcome = $rule();
                } catch (\Throwable $e) {
                    // One malformed rule must not stop the rest — the same
                    // contract RuleEvaluator holds for its own rules.
                    $this->warn('  Rule failed: '.$e->getMessage());
                    $skipped++;
                ...
                match (true) {
                    ($outcome['created'] ?? false)   => $created++,
                    ($outcome['refreshed'] ?? false) => $refreshed++,
                    default                          => $skipped++,
                };
            }

            $this->line("  Signals created: {$created}");
            $this->line("  Signals refreshed: {$refreshed}");
            $this->line("  Rules not triggered: {$skipped}");
        }

 ...
 * ['created' => bool] shape the five ERP rules in SignalGenerator return, and
 * creates signals and evidence through SignalGenerator's own methods, so every
 ...
 *
 * The rules are attached by DATA, not by organization name — SignalRuleRegistry
...
final class OperationalSignalRules
{
     ...
     */
    public function rulesFor(SignalGenerator $generator, string $tenantId, array $datasets): array
    {
        $rules = [];
     ...
     * whose Time Group is '#VALUE!' — intelligence must be derived from facts,
     * not from someone else's formula.
     *
     * FY2025-26 baseline: 6,945 of 65,268 tickets over 24h (10.6%), of which
     * 412 over 72h.
     */
    private function slaBreaches(SignalGenerator $generator, string $tenantId): array
    {
     ...
     *
     * FY2025-26 baseline: 44,500 of 65,268 tickets — 68% — have an empty
     ...
     */
    private function unrecordedRootCause(SignalGenerator $generator, string $tenantId): array
    {
        $since = $this->records->latestPeriod($tenantId, 'complaint');
        ...
        return $generator->raise($tenantId, [
            ...
            // that confidence is computed from what is known — and here
            // everything is known.
            'confidence'     => 1.0,
            'metadata'       => [
     ...
     * actually fixed" — and the two look identical in the SLA report.
     */
    private function repeatComplaintSubscribers(SignalGenerator $generator, string $tenantId): array
    {
        $since = $this->records->latestPeriod($tenantId, 'complaint');
     ...
     */
    private function zoneConcentration(SignalGenerator $generator, string $tenantId): array
    {
        $since = $this->records->latestPeriod($tenantId, 'complaint');
     ...
     */
    private function stalledWorkOrders(SignalGenerator $generator, string $tenantId): array
    {
     ...
     * laying issues (116) and permission issues (49) — both operational, not
     ...
     */
    private function provisioningCancellations(SignalGenerator $generator, string $tenantId): array
    {
     ...
     * calls — a 61% drop rate. A subscriber who cannot reach the help desk
     ...
     */
    private function callDropRate(SignalGenerator $generator, string $tenantId): array
    {
